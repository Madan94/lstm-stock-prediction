"""Script to pre-train all models for all indices."""
...
def main(skip_if_exists: bool = False):
    ...
    Args:
        skip_if_exists: If True, skip training for indices that already have results
    """
    # Create directories
    ...
    for index_name, df in all_data.items():
        ...
        if skip_if_exists:
            ...
            if os.path.exists(results_file):
                logger.info(f"Skipping {index_name} - results already exist at {results_file}")
                continue
